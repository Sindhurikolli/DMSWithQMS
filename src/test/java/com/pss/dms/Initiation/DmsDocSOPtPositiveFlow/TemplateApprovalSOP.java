 ...
 * @author Jeevan
 */
public class TemplateApprovalSOP extends QMSLoginDetails {

	public TemplateApprovalSOP() {
	}
	...
	//
	@Test
	public void toDoTemplateApprovalSOP() throws InterruptedException, IOException, DocumentException, Exception {
		try {

			document = new Document();
			Font font = new Font(Font.FontFamily.TIMES_ROMAN);
			output = System.getProperty("user.dir") + "\\" + "/TestReport/" + "TemplateApprovalSOP"
			...
			writer.open();
			HeaderFooterPageEvent event = new HeaderFooterPageEvent("TemplateApprovalSOP", "PSS-DMS-002", "Pass");
...
//        Thread.sleep(4000);
			...
			document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Click on Template sub menu", sno,
					false);
			Thread.sleep(4000);
			sno++;
	...
	private void methodToSelectRecordApproveTemplateSOP() throws Exception {
		sno++;
		boolean isRecordSelectedApproveTemplateSOP = false;
		int count = 0;
		String templateName = properties.getProperty("TEMPLATE_NAME_CREATE_DOCT_SOP");
		isRecordSelectedApproveTemplateSOP = selectRecordApproveTemplateSop(templateName,
				isRecordSelectedApproveTemplateSOP, count);
		document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Select the record", sno, false);
		Thread.sleep(1000);
		sno++;
		if (isRecordSelectedApproveTemplateSOP) {
	...
	private boolean selectRecordApproveTemplateSop(String templateName, boolean isRecordSelectedApproveTemplateSOP,
		...
		if (perPageNoOfRecordsPresent > 0) {
			while (noOfRecordsChecked < totalNoOfRecords) {
					...
					for (int i = 1; i <= perPageNoOfRecordsPresent; i++) {
						...
						if (templateName.equalsIgnoreCase(templateNameInApproval)) {
							driver.findElement(By.xpath(
									...
									.click();
							isRecordSelectedApproveTemplateSOP = true;
							break;
						}
					}
					if (isRecordSelectedApproveTemplateSOP) {
				...
				if ((!isRecordSelectedApproveTemplateSOP) && (noOfRecordsChecked < totalNoOfRecords)) {
		...
		return isRecordSelectedApproveTemplateSOP;
	}
