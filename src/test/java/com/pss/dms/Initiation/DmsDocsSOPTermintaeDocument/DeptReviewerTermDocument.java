...
public class DeptReviewerTermDocument extends QMSLoginDetails {

	@Test
	public void toDeptReviewerTermDocument() throws InterruptedException, IOException, DocumentException, Exception {
		try {

			document = new Document();
			Font font = new Font(Font.FontFamily.TIMES_ROMAN);
			output = System.getProperty("user.dir") + "\\" + "/TestReport/" + "DeptReviewerTermDocument"
			...
			writer.open();
			HeaderFooterPageEvent event = new HeaderFooterPageEvent("DeptReviewerTermDocument", "PSS-DMS-006", "Pass");
			...
			document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Click on Terminate document menu ", sno,
					false);
			sno++;
			Thread.sleep(16000);
	...
	private void methodToSelectChgDoctRecordDeptReviewApprovalFlow() throws Exception {
		int count = 0;
		String documentDeptSOPReview = properties.getProperty("DOCUMENT_NAME_SOP_TERM_DOCT");
		...
		if (isRecordSelectedForDeptReview) {
			...
			Thread.sleep(1000);
			driver.findElement(By.id("commentsInReviseDocReviewPage")).sendKeys(properties.getProperty("COMMENTS"));
