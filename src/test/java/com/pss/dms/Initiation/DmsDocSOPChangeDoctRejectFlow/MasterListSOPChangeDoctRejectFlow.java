 ...
 * @author Jeevan
 */
public class MasterListSOPChangeDoctRejectFlow extends QMSLoginDetails {

	public MasterListSOPChangeDoctRejectFlow() {
	}
	...
	@Test
	public void toDoMasterListSOPChangeDoctRejectFlow() throws InterruptedException {
		Thread.sleep(1000);
		driver.findElement(By.id("bmrHomeInCommonHPHeader")).click();
		Thread.sleep(1000);
		driver.findElement(By.id("masterListTabIdInDMS")).click();
		Thread.sleep(1000);
		methodToViewMasterListChangeDoctSOPRejectFlow();

	}

	private void methodToViewMasterListChangeDoctSOPRejectFlow() throws InterruptedException {
		driver.findElement(By.id("tree_1_span")).click();
		Thread.sleep(1000);
		boolean isRecordSelected = false;
		int count = 0;
		String doctName = properties.getProperty("DOCUMENT_NAME_SOP_DOCT_REQUEST_CHG_DOCT_REJECT_FLOW");
		String applicationWindow = driver.getWindowHandle();
		Thread.sleep(1000);
		isRecordSelected = selectRecordToViewMasterListChgDoctSOPRejectFlow(doctName, isRecordSelected, count);
		Thread.sleep(1000);
		if (isRecordSelected) {
	...
	private boolean selectRecordToViewMasterListChgDoctSOPRejectFlow(String doctName, boolean isRecordSelected,
