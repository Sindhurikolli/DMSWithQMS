...
public class RejectedDeptReviewerApprovalChngDocRejflow extends QMSLoginDetails {

	@Test
	public void toRejectedDeptReviewerApprovalChngDocRejflow()
		...
		try {
			...
			output = System.getProperty("user.dir") + "\\" + "/TestReport/"
					+ "RejectedDeptReviewerApprovalChngDocRejflow" + (new Random().nextInt()) + ".pdf";
			...
			writer.open();
			HeaderFooterPageEvent event = new HeaderFooterPageEvent("RejectedDeptReviewerApprovalChngDocRejflow",
					"PSS-DMS-020", "Pass");
			...
			methodToSelectChgDoctRecordDeptReviewApprovalFlow();
			document.close();
			writer.close();
	...
	private void methodToSelectChgDoctRecordDeptReviewApprovalFlow() throws Exception {
		int count = 0;
		...
		isRecordSelectedForDeptReview = selectChgDoctSOPApprovalFlowRecord(documentDeptSOPReview,
		...
		if (isRecordSelectedForDeptReview) {
			...
			document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Click on continue Approval button", sno,
					false);
			Thread.sleep(9000);
			...
			for (String pdfWindows : pdfWindow) {
				...
				if (!applicationWindow.equalsIgnoreCase(pdfWindows)) {
					...
					if (exists) {
						...
						document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Click On Forward button",
								sno, false);
						Thread.sleep(1000);
	...
	private boolean selectChgDoctSOPApprovalFlowRecord(String documentDeptSOPReview,
