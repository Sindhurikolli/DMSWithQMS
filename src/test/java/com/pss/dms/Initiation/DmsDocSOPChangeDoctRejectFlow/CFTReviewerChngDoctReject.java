 ...
 * @author Jeevan
 */
public class CFTReviewerChngDoctReject extends QMSLoginDetails {
	...
	public void toCFTReviewerChngDoctReject() throws InterruptedException, IOException, DocumentException, Exception {
		try {
			...
			output = System.getProperty("user.dir") + "\\" + "/TestReport/" + "CFTReviewerChngDoctReject"
			...
			writer.open();
			HeaderFooterPageEvent event = new HeaderFooterPageEvent("CFTReviewerChngDoctReject", "PSS-DMS-028", "Pass");
			...
			metodToDoCFTReviewerChgSOPDoctRejectFlow();
			document.close();
			writer.close();
	...
	private void metodToDoCFTReviewerChgSOPDoctRejectFlow() throws Exception {
		int count = 0;
		...
		isRecordSelectedForCFTReview = selectRecordCFTReviewStageChgSOPRejectFlow(documentDeptSOPReview,
				...
				if (!applicationWindow.equalsIgnoreCase(pdfWindows)) {
					...
					if (exists) {
						...
						if (webElementList.size() > 0) {
							stagesDownList.selectByIndex(Integer.parseInt("1"));// Initiator
	...
	private boolean selectRecordCFTReviewStageChgSOPRejectFlow(String documentDeptSOPReview,
