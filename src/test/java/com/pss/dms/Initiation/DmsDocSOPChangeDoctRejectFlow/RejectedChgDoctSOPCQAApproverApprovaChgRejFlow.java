 ...
 * @author Jeevan
 */
public class RejectedChgDoctSOPCQAApproverApprovaChgRejFlow extends QMSLoginDetails {

	public RejectedChgDoctSOPCQAApproverApprovaChgRejFlow() {
	}
	...
	@Test
	public void toDoRejectedChgDoctSOPCQAApproverApprovaChgRejFlow() throws InterruptedException {
		Thread.sleep(1000);
		...
		Thread.sleep(1000);
		methodToSelectRecordCQAApproverChgRejFlowSOP();
		Thread.sleep(1000);
		driver.manage().deleteAllCookies();
		Thread.sleep(1000);
	}

	private void methodToSelectRecordCQAApproverChgRejFlowSOP() throws InterruptedException {
		int count = 0;
		String documentDeptSOPReview = properties.getProperty("DOCUMENT_NAME_SOP_DOCT_REQUEST_CHG_DOCT_REJECT_FLOW");
		boolean isRecordSelectedForCQAApprover = false;
		isRecordSelectedForCQAApprover = selectRecQAApproverApprovalSOPChgRejFlow(documentDeptSOPReview,
				isRecordSelectedForCQAApprover, count);
		Thread.sleep(1000);
		if (isRecordSelectedForCQAApprover) {
	...
	private boolean selectRecQAApproverApprovalSOPChgRejFlow(String documentDeptSOPReview,
			boolean isRecordSelectedForCQAApprover, int count) throws InterruptedException {
		...
		if (perPageNoOfRecordsPresent > 0) {
			while (noOfRecordsChecked < totalNoOfRecords) {
					...
					for (int i = 1; i <= perPageNoOfRecordsPresent; i++) {
						...
						if (documentDeptSOPReview.equalsIgnoreCase(newDoctReqNameInApproval)) {
							driver.findElement(
									...
									.click();
							isRecordSelectedForCQAApprover = true;
							break;
						}
					}
					if (isRecordSelectedForCQAApprover) {
				...
				if ((!isRecordSelectedForCQAApprover) && (noOfRecordsChecked < totalNoOfRecords)) {
		...
		return isRecordSelectedForCQAApprover;
	}
}
