 ...
 * @author Jeevan
 */
public class RejectedQAReviewerApprovalChangedDoctRejFlow extends QMSLoginDetails {

	@Test
	public void toQAReviewerChngDoc() throws InterruptedException, IOException, DocumentException, Exception {
		try {

			document = new Document();
			Font font = new Font(Font.FontFamily.TIMES_ROMAN);
			output = System.getProperty("user.dir") + "\\" + "/TestReport/"
					+ "RejectedQAReviewerApprovalChangedDoctRejFlow" + (new Random().nextInt()) + ".pdf";
			...
			writer.open();
			HeaderFooterPageEvent event = new HeaderFooterPageEvent("RejectedQAReviewerApprovalChangedDoctRejFlow",
					"PSS-DMS-027", "Pass");
			...
			methodToSelectRecordQAReviewRejFlowSOP();
			document.close();
			writer.close();
	...
	private void methodToSelectRecordQAReviewRejFlowSOP() throws Exception {
		int count = 0;
		...
		isRecordSelectedForQAReview = selectRecQAReviewApprovalSOPChgRejFlow(documentDeptSOPReview,
			...
			for (String pdfWindows : pdfWindow) {
				...
				if (!applicationWindow.equalsIgnoreCase(pdfWindows)) {
					...
					if (exists) {
						...
						Thread.sleep(1000);
						driver.findElement(By.id("continueBtnInChngDocRevForm")).click();
						document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Click on Contniue button",
								sno, false);
						Thread.sleep(9000);
	...
	private boolean selectRecQAReviewApprovalSOPChgRejFlow(String documentDeptSOPReview,
