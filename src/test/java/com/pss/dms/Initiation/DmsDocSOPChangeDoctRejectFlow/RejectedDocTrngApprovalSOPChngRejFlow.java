...
public class RejectedDocTrngApprovalSOPChngRejFlow extends QMSLoginDetails {
	@Test
	public void toDocTrngNotificationListChngDoc()
		...
		try {
			...
			output = System.getProperty("user.dir") + "\\" + "/TestReport/" + "RejectedDocTrngApprovalSOPChngRejFlow"
			...
			writer.open();
			HeaderFooterPageEvent event = new HeaderFooterPageEvent("RejectedDocTrngApprovalSOPChngRejFlow",
					"PSS-DMS-033", "Pass");
			...
			document = Utilities.getScreenShotAndAddInLogDoc(driver, document,
					"Click on Resubmitted rejected radio button list menu", sno, false);
			Thread.sleep(4000);
			WebDriverWait wait1 = new WebDriverWait(driver, 460);
			wait1.until(ExpectedConditions.invisibilityOf(driver
					.findElement(By.cssSelector("#docTrainingRejectedGridStore > div > div.jtable-busy-message"))));
	...
	private void methodToSelectRecordTrainingNotificationListSOPChgDoctPositiveFlw() throws Exception {
		...
		int count = 0;
		String documentNameInNotification = properties.getProperty("DOCUMENT_NAME_SOP_DOCT_REQUEST_IN_REJECT_FLOW");
			...
			while (noOfRecordsChecked < totalNoOfRecords) {
				if (totalNoOfRecords > 1) {
					for (int i = 1; i <= perPageNoOfRecordsPresent; i++) {
						String doctNameInApproval = driver.findElement(
