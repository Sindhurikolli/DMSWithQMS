 ...
 * @author Jeevan
 */
public class QAReviewerChgDoctReject extends QMSLoginDetails {

	@Test
	public void toQAReviewerChgDoctReject() throws InterruptedException, IOException, DocumentException, Exception {
		try {

			document = new Document();
			Font font = new Font(Font.FontFamily.TIMES_ROMAN);
			output = System.getProperty("user.dir") + "\\" + "/TestReport/" + "QAReviewerChgDoctReject"
			...
			writer.open();
			HeaderFooterPageEvent event = new HeaderFooterPageEvent("QAReviewerChgDoctReject", "PSS-DMS-021", "Pass");
			...
			methodToDoQAReviewerChgSOPDoctRejFlow();
			document.close();
			writer.close();
	...
	private void methodToDoQAReviewerChgSOPDoctRejFlow() throws Exception {
		int count = 0;
		String documentDeptSOPReview = properties.getProperty("DOCUMENT_NAME_SOP_DOCT_REQUEST_IN_REJECT_FLOW");
		boolean isRecordSelectedForQAReview = false;
		isRecordSelectedForQAReview = selectRecordQAReviewStageChgRejectFlow(documentDeptSOPReview,
						...
						for (String pdfs : pdf) {
							...
							if (!applicationWindow.equalsIgnoreCase(pdfs)) {
								...
								driver.findElement(By.cssSelector(
										...
										.click();
								document = Utilities.getScreenShotAndAddInLogDoc(driver, document,
										"Click on Create note", sno, false);
								Thread.sleep(1000);
								driver.findElement(By.cssSelector(
										"#toolbar_documentViewer_annotations_flowpaper_notetypeselector_point"))
										.click();
								document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Click on Point",
										sno, false);
								Thread.sleep(1000);
								driver.findElement(By.cssSelector("#page_0_documentViewer_canvasOverlay")).click();
								document = Utilities.getScreenShotAndAddInLogDoc(driver, document,
										"Click on Document page", sno, false);
								Thread.sleep(3000);
								if (driver.findElement(By.className("flowpaper_note_container")).isDisplayed()) {
									driver.findElement(By.className("flowpaper_note_textarea"))
											.sendKeys(properties.getProperty("COMMENTS").toString());
									document = Utilities.getScreenShotAndAddInLogDoc(driver, document,
											"Enter the comments", sno, false);
									Thread.sleep(1000);
								}
								// Selecting StageWise DropDown
								Select stagesDownList = Helper.getSelectInstance(driver, By.id("stagesOfLCToMove"));
								...
								driver.findElement(By.id("rejectBtnDocRevAnnotsWin")).click();
								document = Utilities.getScreenShotAndAddInLogDoc(driver, document,
										"Click on reject button", sno, false);
								Thread.sleep(1000);
	...
	private boolean selectRecordQAReviewStageChgRejectFlow(String documentDeptSOPReview,
					...
					for (int i = 1; i <= perPageNoOfRecordsPresent; i++) {
						String newDoctReqNameInApproval = driver
								.findElement(By.xpath(
										"//*[@id=\"changeDocReviewTable\"]/div/table/tbody/tr[ " + i + " ]/td[2]"))
