 ...
 * @author Jeevan
 */
public class CFTReviewerChngDoc extends QMSLoginDetails {

	@Test
	public void toCFTReviewerChngDoc() throws InterruptedException, IOException, DocumentException, Exception {
		try {

			document = new Document();
			Font font = new Font(Font.FontFamily.TIMES_ROMAN);
			output = System.getProperty("user.dir") + "\\" + "/TestReport/" + "CFTReviewerChngDoc"
			...
			writer.open();
			HeaderFooterPageEvent event = new HeaderFooterPageEvent("CFTReviewerChngDoc", "PSS-DMS-008", "Pass");
			...
			wait1.until(ExpectedConditions.invisibilityOf(
					driver.findElement(By.cssSelector("#changeDocReviewTable > div > div.jtable-busy-message"))));
			metodToDoCFTReviewerChgSOPDoctApprovalFlow();
			document.close();
			writer.close();
	...
	private void metodToDoCFTReviewerChgSOPDoctApprovalFlow() throws Exception {
		int count = 0;
		String documentCFTSOPReview = properties.getProperty("DOCUMENT_NAME_SOP_DOCT_REQUEST");
		boolean isRecordSelectedForCFTReview = false;
		isRecordSelectedForCFTReview = selectRecordCFTReviewSOPChgApprovalFlow(documentCFTSOPReview,
			...
			for (String pdfWindows : pdfWindow) {
				...
				if (!applicationWindow.equalsIgnoreCase(pdfWindows)) {
					...
					if (exists) {
						...
						Thread.sleep(4000);
						sno++;
						driver.findElement(By.xpath("/html/body/div[1]/header/nav/ul/li[13]/a/span")).click();
						...
						for (String pdfs : pdf) {
							...
							if (!applicationWindow.equalsIgnoreCase(pdfs)) {
								...
								Thread.sleep(5000);
								sno++;
								driver.findElement(By.xpath("/html/body/div[1]/header/nav/ul/li[13]/ul/li[3]/a"))
										.click();
	...
	private boolean selectRecordCFTReviewSOPChgApprovalFlow(String documentCFTSOPReview,
		...
		if (perPageNoOfRecordsPresent > 0 && count == 0) {
			if ((totalNoOfRecords > 1)
					&& ((documentCFTSOPReview == null) || ("".equalsIgnoreCase(documentCFTSOPReview)))) {
				documentCFTSOPReview = driver
						.findElement(By.xpath("//*[@id=\"changeDocReviewTable\"]/div/table/tbody/tr[1]/td[2]"))
						.getText();// documentType
			} else if ((documentCFTSOPReview == null) || ("".equalsIgnoreCase(documentCFTSOPReview))) {
				documentCFTSOPReview = driver
			...
			while (noOfRecordsChecked < totalNoOfRecords) {
				if (totalNoOfRecords > 1) {
					for (int i = 1; i <= perPageNoOfRecordsPresent; i++) {
						...
						if (documentCFTSOPReview.equalsIgnoreCase(newDoctReqNameInApproval)) {
							driver.findElement(
