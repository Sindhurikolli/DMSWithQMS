 ...
 * @author Jeevan
 */
public class DoctViewSOPChangeDoctApprovalFlow extends QMSLoginDetails {

	public DoctViewSOPChangeDoctApprovalFlow() {
	}
	...
	@Test
	public void toDoDoctViewSOPChangeDoctApprovalFlow() throws InterruptedException {
		Thread.sleep(1000);
		...
		Thread.sleep(1000);
		methodToDoSearchViewSOPChgDoctApprovalFlow();

	}

	private void methodToDoSearchViewSOPChgDoctApprovalFlow() throws InterruptedException {
		driver.findElement(By.id("dmsDocViewListSearchBtnId")).click();
		Thread.sleep(1000);
		String doctNameView = properties.getProperty("DOCUMENT_NAME_SOP_DOCT_REQUEST_CHG_DOCT_APPROVAL_FLOW");
		boolean isRecordSelected = false;
		int count = 0;
		String applicationWindow = driver.getWindowHandle();
		Thread.sleep(1000);
		isRecordSelected = selectDoctRecdViewSOPChgDoctApprovalFlow(doctNameView, isRecordSelected, count);
		Thread.sleep(2000);
		if (isRecordSelected) {
	...
	private boolean selectDoctRecdViewSOPChgDoctApprovalFlow(String doctNameView, boolean isRecordSelected, int count)
