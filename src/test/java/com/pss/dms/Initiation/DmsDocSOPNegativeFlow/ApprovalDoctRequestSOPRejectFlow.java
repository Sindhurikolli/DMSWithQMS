 ...
 * @author Jeevan
 */
public class ApprovalDoctRequestSOPRejectFlow extends QMSLoginDetails {

	public ApprovalDoctRequestSOPRejectFlow() {
	}
	...
	@Test
	public void toDoApprovalDoctRequestSOPRejectFlow()
			throws InterruptedException, IOException, DocumentException, Exception {
		try {

			document = new Document();
			Font font = new Font(Font.FontFamily.TIMES_ROMAN);
			output = System.getProperty("user.dir") + "\\" + "/TestReport/" + "toDoApprovalDoctRequestSOPRejectFlow"
			...
			writer.open();
			HeaderFooterPageEvent event = new HeaderFooterPageEvent("toDoApprovalDoctRequestSOPRejectFlow",
					"PSS-DMS-016", "Pass");
			...
			Thread.sleep(10000);
			methodToDoDocmtApprovalRequestSOPRejectFlow();
			document.close();
			writer.close();
	...
	private void methodToDoDocmtApprovalRequestSOPRejectFlow() throws Exception {
		boolean isRecordSelectedForApprovalReq = false;
		int count = 0;
		String documentNameReq = properties.getProperty("DOCUMENT_NAME_SOP_DOCT_REQUEST");
		isRecordSelectedForApprovalReq = selectRecordDocReqApprovalSOPRejectFlow(documentNameReq,
	...
	private boolean selectRecordDocReqApprovalSOPRejectFlow(String documentNameReq,
