 ...
 * @author Jeevan
 */
public class TrngCompletedSTPChgDoctRejectFlow extends QMSLoginDetails {

	public TrngCompletedSTPChgDoctRejectFlow() {
	}
	...
	@Test
	public void toDoTrngCompletedSTPChgDoctRejectFlow() throws InterruptedException {
		Thread.sleep(1000);
		...
		Thread.sleep(1000);
		methodToSelectRecordTrngCompletedListSTPDoctChgRejFlow();
		Thread.sleep(1000);
	...
	private void methodToSelectRecordTrngCompletedListSTPDoctChgRejFlow() throws InterruptedException {
		boolean isRecordSelectedTrngCompletedList = false;
		int count = 0;
		String documentNameTrngCompleted = properties
				.getProperty("DOCUMENT_NAME_STP_DOCT_REQUEST_CHG_DOCT_REJECT_FLOW");
		Thread.sleep(1000);
		isRecordSelectedTrngCompletedList = selectRecordTrngCompletedSTPChangeDoctRejFlow(documentNameTrngCompleted,
		...
		if (isRecordSelectedTrngCompletedList) {
			...
			Thread.sleep(1000);
			selectUserGroupMethodForTrngCompletedListInSTPDoct();
	...
	private boolean selectRecordTrngCompletedSTPChangeDoctRejFlow(String documentNameTrngCompleted,
	...
	private void selectUserGroupMethodForTrngCompletedListInSTPDoct() throws InterruptedException {
		boolean isRecordSelectedForTrngCompletedList = false;
		int count = 0;
		String userGrpName = properties.getProperty("USER_GROUP_TRNG_COMPLETED_LIST");
		isRecordSelectedForTrngCompletedList = selectRecordForTrngCompletedListUserGrpSTP(userGrpName,
				isRecordSelectedForTrngCompletedList, count);
		Thread.sleep(1000);
		if (isRecordSelectedForTrngCompletedList) {
			...
			System.out.println("User Group Record is not available");
			driver.findElement(By.xpath("/html/body/div[17]/div[3]/div/button[2]/span")).click();
			Thread.sleep(1000);
		}
	}

	private boolean selectRecordForTrngCompletedListUserGrpSTP(String userGrpName,
			boolean isRecordSelectedForTrngCompletedList, int count) {
		WebElement table = driver.findElement(By.id("userGroupsTable"));
		WebElement tableBody = table.findElement(By.tagName("tbody"));
		int perPageNoOfRecordsPresent = tableBody.findElements(By.tagName("tr")).size();
		int noOfRecordsChecked = 0;
		if (perPageNoOfRecordsPresent > 0 && count == 0) {
			if (/* (totalNoOfRecords > 1) && */((userGrpName == null) || ("".equalsIgnoreCase(userGrpName)))) {
				userGrpName = driver
						.findElement(By.xpath("//*[@id=\"userGroupsTable\"]/div/div[4]/table/tbody/tr[1]/td[2]"))
						.getText();// documentType
			} else if ((userGrpName == null) || ("".equalsIgnoreCase(userGrpName))) {
				userGrpName = driver
				...
				if (perPageNoOfRecordsPresent > 1) {
					for (int i = 1; i <= perPageNoOfRecordsPresent; i++) {
						String newDoctReqNameInApproval = driver
								...
								.getText();// documentTypeName
						if (userGrpName.equalsIgnoreCase(newDoctReqNameInApproval)) {
							driver.findElement(
									By.xpath("//*[@id=\"userGroupsTable\"]/div/div[4]/table/tbody/tr[" + i + "]/td[2]"))
									.click();
							isRecordSelectedForTrngCompletedList = true;
							break;
						}
					}
					if (isRecordSelectedForTrngCompletedList) {
		...
		return isRecordSelectedForTrngCompletedList;
	}
}
