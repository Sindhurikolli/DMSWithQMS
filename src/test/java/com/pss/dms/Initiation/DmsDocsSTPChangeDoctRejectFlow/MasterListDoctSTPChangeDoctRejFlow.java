 ...
 * @author Jeevan
 */
public class MasterListDoctSTPChangeDoctRejFlow extends QMSLoginDetails {

	public MasterListDoctSTPChangeDoctRejFlow() {
	}
	...
	@Test
	public void toDoMasterListDoctSTPChangeDoctRejFlow() throws InterruptedException {
		Thread.sleep(1000);
		driver.findElement(By.id("bmrHomeInCommonHPHeader")).click();
		Thread.sleep(1000);
		driver.findElement(By.id("masterListTabIdInDMS")).click();
		Thread.sleep(1000);
		methodToViewMasterListChangeDoctSTPPositiveFlow();

	}

	private void methodToViewMasterListChangeDoctSTPPositiveFlow() throws InterruptedException {
		driver.findElement(By.id("tree_1_span")).click();
		Thread.sleep(1000);
		boolean isRecordSelected = false;
		int count = 0;
		String doctName = properties.getProperty("DOCUMENT_NAME_STP_DOCT_REQUEST_CHG_DOCT_REJECT_FLOW");
		String applicationWindow = driver.getWindowHandle();
		Thread.sleep(1000);
		isRecordSelected = selectRecordToViewMasterListChgDoctSTP(doctName, isRecordSelected, count);
		Thread.sleep(1000);
		if (isRecordSelected) {
	...
	private boolean selectRecordToViewMasterListChgDoctSTP(String doctName, boolean isRecordSelected, int count)
