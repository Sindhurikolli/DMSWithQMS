 ...
 * @author Jeevan
 */
public class MasterListSTPChangeDoctPositiveFlow extends QMSLoginDetails {

	public MasterListSTPChangeDoctPositiveFlow() {
	}
	...
	//
	@Test
	public void toDoMasterListSTPChangeDoctPositiveFlow() throws InterruptedException {
		Thread.sleep(1000);
		driver.findElement(By.id("bmrHomeInCommonHPHeader")).click();
		Thread.sleep(1000);
		driver.findElement(By.id("masterListTabIdInDMS")).click();
		Thread.sleep(1000);
		methodToViewMasterListDoctSTPPositiveFlow();

	}

	private void methodToViewMasterListDoctSTPPositiveFlow() throws InterruptedException {
		driver.findElement(By.id("tree_1_span")).click();
		Thread.sleep(1000);
		boolean isRecordSelected = false;
		int count = 0;
		String doctName = properties.getProperty("DOCUMENT_NAME_STP_DOCT_REQUEST_CHG_DOCT_APPROVAL_FLOW");
		String applicationWindow = driver.getWindowHandle();
		Thread.sleep(1000);
		isRecordSelected = selectRecordToViewMasterListDoctSTPPositiveFlow(doctName, isRecordSelected, count);
		Thread.sleep(1000);
		if (isRecordSelected) {
	...
	private boolean selectRecordToViewMasterListDoctSTPPositiveFlow(String doctName, boolean isRecordSelected,
