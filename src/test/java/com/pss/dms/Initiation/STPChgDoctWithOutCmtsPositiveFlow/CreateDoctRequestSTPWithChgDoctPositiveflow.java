 ...
 * @author Jeevan
 */
public class CreateDoctRequestSTPWithChgDoctPositiveflow extends QMSLoginDetails {

	public CreateDoctRequestSTPWithChgDoctPositiveflow() {
	}
	...
	@Test
	public void toDoCreateDoctRequestSTPWithChgDoctPositiveflow() throws InterruptedException {
		Thread.sleep(1000);
		...
		Thread.sleep(1000);
		methodToDoCreateDoctRequestSTPWithPositiveFlow();
	}

	private void methodToDoCreateDoctRequestSTPWithPositiveFlow() throws InterruptedException {
		driver.findElement(By.id("documentNameInNewDocRequestPage"))
				.sendKeys(properties.getProperty("DOCUMENT_NAME_STP_DOCT_REQUEST_CHG_DOCT_APPROVAL_FLOW"));
		Thread.sleep(1000);
		...
		if (webElementList.size() > 0) {
			rolesDropDownList.selectByIndex(Integer.parseInt("2"));// STP
		} else {
			System.out.println("No options in Select(Roles - drop down list)");
		}
		Thread.sleep(1000);
		methodToSelectSingleApprovalInDoctReqSTPWithChgDoctFlow();
	}

	private void methodToSelectSingleApprovalInDoctReqSTPWithChgDoctFlow() throws InterruptedException {
		driver.findElement(By.id("selectRepToInCreateUser")).click();
		Thread.sleep(1000);
		boolean isRecordSelectedSingleApprovalDoctReqSTP = false;
		...
		isRecordSelectedSingleApprovalDoctReqSTP = Helper.selectSingleApprovalRecord(driver, userSingleApprovalName,
				isRecordSelectedSingleApprovalDoctReqSTP, count);
		Thread.sleep(1000);
		if (isRecordSelectedSingleApprovalDoctReqSTP) {
