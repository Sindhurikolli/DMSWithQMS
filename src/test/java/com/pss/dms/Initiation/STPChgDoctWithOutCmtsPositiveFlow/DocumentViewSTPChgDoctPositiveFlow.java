 ...
 * @author Jeevan
 */
public class DocumentViewSTPChgDoctPositiveFlow extends QMSLoginDetails {

	public DocumentViewSTPChgDoctPositiveFlow() {
	}
	...
	@Test
	public void toDoDocumentViewSTPChgDoctPositiveFlow() throws InterruptedException {
		Thread.sleep(1000);
		...
		Thread.sleep(1000);
		methodToDoSearchViewSTPChgDoctApprovalFlow();

	}

	private void methodToDoSearchViewSTPChgDoctApprovalFlow() throws InterruptedException {
		driver.findElement(By.id("dmsDocViewListSearchBtnId")).click();
		Thread.sleep(1000);
		String doctNameView = properties.getProperty("DOCUMENT_NAME_STP_DOCT_REQUEST_CHG_DOCT_APPROVAL_FLOW");
		boolean isRecordSelected = false;
		int count = 0;
		String applicationWindow = driver.getWindowHandle();
		Thread.sleep(1000);
		isRecordSelected = selectDoctRecdViewSTPChgDoctApprovalFlow(doctNameView, isRecordSelected, count);
		Thread.sleep(2000);
		if (isRecordSelected) {
	...
	private boolean selectDoctRecdViewSTPChgDoctApprovalFlow(String doctNameView, boolean isRecordSelected, int count)
