...
public class RejectedDocumentTrainingNotificationAfterReject extends QMSLoginDetails {

	@Test
	public void toDoRejectedDocumentTrainingNotificationAfterReject()
			throws InterruptedException, IOException, DocumentException, Exception {
//		try {

		document = new Document();
		Font font = new Font(Font.FontFamily.TIMES_ROMAN);
		output = System.getProperty("user.dir") + "\\" + "/TestReport/" + "RejectedDoctTrainingApprovalSOPRejectedFlow"
		...
		writer.open();
		HeaderFooterPageEvent event = new HeaderFooterPageEvent("RejectedDoctTrainingApprovalSOPRejectedFlow",
				"PSS-DMS-025", "Pass");
...
//        driver.findElement(By.cssSelector("a[href='dmsTrngNotfListPage.do']")).click();
		...
		Thread.sleep(1000);
		sno++;
		wait1.until(ExpectedConditions.elementToBeClickable(By.id("resubmitTraingNotfListAction")));
		driver.findElement(By.id("resubmitTraingNotfListAction")).click();
		document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Click on Resubmitted rejected radio button",
				sno, false);
		Thread.sleep(1000);
		wait1.until(ExpectedConditions.presenceOfElementLocated(By
				.cssSelector("#docTrainingRejectedGridStore > div > div.jtable-busy-message[style='display: none;']")));
		methodToSelectRecordTrainingNotificationListSOPRejectFlow();
		document.close();
		writer.close();
...
//    }
	...
	private void methodToSelectRecordTrainingNotificationListSOPRejectFlow() throws Exception {
		...
		isRecordSelectedTrainingNotification = selectRecordToDOTrngApprovalSOPRejectFlow(documentNameInNotification,
				isRecordSelectedTrainingNotification, count);
	...
	private boolean selectRecordToDOTrngApprovalSOPRejectFlow(String doctNameTrngApprovalSOP,
		...
		WebElement table = driver.findElement(By.id("docTrainingRejectedGridStore"));
		...
		if (perPageNoOfRecordsPresent > 0 && count == 0) {
			if ((totalNoOfRecords > 1)
				...
				doctNameTrngApprovalSOP = driver
						.findElement(
								By.xpath("//*[@id=\"docTrainingRejectedGridStore\"]/div/div[4]/table/tbody/tr/td[2]"))
			...
			while (noOfRecordsChecked < totalNoOfRecords) {
				if (totalNoOfRecords > 1) {
					for (int i = 1; i <= perPageNoOfRecordsPresent; i++) {
						String newDoctReqNameInApproval = driver.findElement(
								By.xpath("//*[@id=\"docTrainingRejectedGridStore\"]/div/div[4]/table/tbody/tr[ " + i
										+ " ]/td[2]"))
					...
					String newDoctReqNameInApproval = driver
							.findElement(By
									.xpath("//*[@id=\"docTrainingRejectedGridStore\"]/div/div[4]/table/tbody/tr/td[2]"))
							.getText();
					if (doctNameTrngApprovalSOP.equalsIgnoreCase(newDoctReqNameInApproval)) {
						driver.findElement(
								By.xpath("//*[@id=\"docTrainingRejectedGridStore\"]/div/div[4]/table/tbody/tr/td[2]"))
				...
				if ((!isRecordSelectedTrngApprovalSOP) && (noOfRecordsChecked < totalNoOfRecords)) {
					driver.findElement(By.className("jtable-page-number-next")).click();// next page in Document approve
																						// list
					Thread.sleep(3000);
					table = driver.findElement(By.id("docTrainingRejectedGridStore"));// Document Tree approve table
