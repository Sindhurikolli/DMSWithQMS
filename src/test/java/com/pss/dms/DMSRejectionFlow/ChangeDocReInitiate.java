...
public class ChangeDocReInitiate extends QMSLoginDetails {

	@Test
	public void toChangeDocReInitiate() throws InterruptedException, IOException, DocumentException, Exception {
//		try {

		document = new Document();
		Font font = new Font(Font.FontFamily.TIMES_ROMAN);
		output = System.getProperty("user.dir") + "\\" + "/TestReport/" + "ChangeDocReInitiate"
		...
		writer.open();
		HeaderFooterPageEvent event = new HeaderFooterPageEvent("ChangeDocReInitiate", "PSS-DMS-019", "Pass");
		...
		Thread.sleep(2000);
		sno++;
		wait1.until(ExpectedConditions.elementToBeClickable(By.id("rejectSelectionInChangeDocReq")));
		...
		wait1.until(ExpectedConditions.presenceOfElementLocated(By.cssSelector(
				"#rejectedChangeDocInitiateReqGridId > div > div.jtable-busy-message[style='display: none;']")));
		todoChangeDocReInitiate();
		document.close();
		writer.close();
...
//    }
	...
	private void todoChangeDocReInitiate() throws Exception {
		sno++;
		int count = 0;
...
//        	JavascriptExecutor jse = (JavascriptExecutor) driver;
			...
			Thread.sleep(4000);
			sno++;
//            driver.findElement(By.id("versionNoOfDocumentUpload")).sendKeys(properties.getProperty("VERSION_NO_INITIATE_NEW_DOCT"));
...
//            sno++;
			...
			document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Click on Next button", sno, false);
			Thread.sleep(5000);
//            wait1.until(ExpectedConditions.presenceOfElementLocated(By.cssSelector("style='display: none;'")));
			sno++;
			driver.findElement(By.linkText("Next")).click();
			document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Click on Next button", sno, false);
			Thread.sleep(2000);
