...
public class ChangeDocumentRequestResubmit extends QMSLoginDetails {

	@Test
	public void CreateDocumentRequestResubmitMethod()
			throws InterruptedException, IOException, DocumentException, Exception {
//		try {

		document = new Document();
		Font font = new Font(Font.FontFamily.TIMES_ROMAN);
		output = System.getProperty("user.dir") + "\\" + "/TestReport/" + "CreateDocumentRequestResubmit"
		...
		writer.open();
		HeaderFooterPageEvent event = new HeaderFooterPageEvent("CreateDocumentRequestResubmit", "PSS-DMS-003", "Pass");
...
//			WebDriverWait wait = new WebDriverWait(driver, 440);
		...
		document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Click on change Request", sno, false);
		Thread.sleep(2000);
		...
		wait1.until(ExpectedConditions.presenceOfElementLocated(
				By.cssSelector("#rejectedChangeDocReqGridId > div > div.jtable-busy-message[style='display: none;']")));
		toCreateDocumentRequestResubmitMethod();
		document.close();
		writer.close();
...
//	}
	...
	private void toCreateDocumentRequestResubmitMethod() throws Exception {
		Thread.sleep(1000);
		sno++;
		...
		if (isRecordSelectedForDocumentRequest) {
			...
			driver.findElement(By.id("rejOrTerminateCommentsInChangeDocReq"))
					.sendKeys(properties.getProperty("ReSubmitted"));
	...
	private boolean selectRecordDocReqApproval(String documentNameReq, boolean isRecordSelectedForDocumentRequest,
		...
		WebElement table = driver.findElement(By.id("rejectedChangeDocReqGridId"));
		...
		if (perPageNoOfRecordsPresent > 0 && count == 0) {
			if ((totalNoOfRecords > 1) && ((documentNameReq == null) || ("".equalsIgnoreCase(documentNameReq)))) {
				documentNameReq = driver
						.findElement(
								By.xpath("//*[@id=\"rejectedChangeDocReqGridId\"]/div/div[4]/table/tbody/tr[1]/td[3]"))
						.getText();// documentType
			} else if ((documentNameReq == null) || ("".equalsIgnoreCase(documentNameReq))) {
				documentNameReq = driver
						.findElement(
								By.xpath("//*[@id=\"rejectedChangeDocReqGridId\"]/div/div[4]/table/tbody/tr/td[3]"))
						.getText();// documentType
			}
			++count;
		}
			...
			while (noOfRecordsChecked < totalNoOfRecords) {
				if (totalNoOfRecords > 1) {
					for (int i = 1; i <= perPageNoOfRecordsPresent; i++) {
						String newDoctReqNameInApproval = driver.findElement(By.xpath(
								"//*[@id=\"rejectedChangeDocReqGridId\"]/div/div[4]/table/tbody/tr[" + i + "]/td[3]"))
								.getText();// documentTypeName
						if (documentNameReq.equalsIgnoreCase(newDoctReqNameInApproval)) {
							driver.findElement(
									By.xpath("//*[@id=\"rejectedChangeDocReqGridId\"]/div/div[4]/table/tbody/tr[" + i
											+ "]/td[3]"))
									.click();
				...
				if ((!isRecordSelectedForDocumentRequest) && (noOfRecordsChecked < totalNoOfRecords)) {
					driver.findElement(By.className("jtable-page-number-next")).click();// next page in Document approve
																						// list
					Thread.sleep(3000);
					table = driver.findElement(By.id("rejectedChangeDocReqGridId"));// Document Tree approve table
