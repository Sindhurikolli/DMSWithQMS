 ...
 * @author Jeevan
 */
public class TerminateDoctInitiate extends QMSLoginDetails {

	@Test
	public void toTerminateDoctInitiate() throws InterruptedException, IOException, DocumentException, Exception {
//		try {

		document = new Document();
		Font font = new Font(Font.FontFamily.TIMES_ROMAN);
		output = System.getProperty("user.dir") + "\\" + "/TestReport/" + "TerminateDoctInitiate"
		...
		writer.open();
		HeaderFooterPageEvent event = new HeaderFooterPageEvent("TerminateDoctInitiate", "PSS-DMS-005", "Pass");
		...
		wait1.until(ExpectedConditions.elementToBeClickable(By.cssSelector("a[href='terminateDocInitiatePage.do']")));
		Thread.sleep(2000);
		driver.findElement(By.cssSelector("a[href='terminateDocInitiatePage.do']")).click();
		Thread.sleep(2000);
		...
		wait1.until(ExpectedConditions.presenceOfElementLocated(By.cssSelector(
				"#terminateDocumentInitiateReqGridId > div > div.jtable-busy-message[style='display: none;']")));
		todoTerminateDoctInitiate();
		document.close();
		writer.close();
...
//	}
	...
	private void todoTerminateDoctInitiate() throws Exception {
		...
		if (isRecordSelectedChgDmtInitiation) {
			...
			Thread.sleep(5000);
			wait.until(ExpectedConditions.presenceOfElementLocated(By.id("reasonForModifyInRejectedEditDocumentform")));
	...
	private boolean selectRecordChgReqDtlsTableApprovalFlow(String documentName,
		...
		WebElement table = driver.findElement(By.id("terminateDocumentInitiateReqGridId"));
		WebElement tableBody = table.findElement(By.tagName("tbody"));
		int perPageNoOfRecordsPresent = tableBody.findElements(By.tagName("tr")).size();
		int totalNoOfRecords = 0;
		int noOfRecordsChecked = 0;
		if (perPageNoOfRecordsPresent > 0) {// *[@id="terminateDocumentInitiateReqGridId"]/div/div[4]/table/tbody/tr/td[3]
			...
			if ((totalNoOfRecords > 1) && ((documentName == null) || ("".equalsIgnoreCase(documentName)))) {
				documentName = driver
						.findElement(By.xpath(
								"//*[@id=\"terminateDocumentInitiateReqGridId\"]/div/div[4]/table/tbody/tr[1]/td[3]"))
						.getText();// documentType
			} else if ((documentName == null) || ("".equalsIgnoreCase(documentName))) {
				documentName = driver
						.findElement(By.xpath(
								"//*[@id=\"terminateDocumentInitiateReqGridId\"]/div/div[4]/table/tbody/tr/td[3]"))
						.getText();// documentType
			}
			++count;
		}
		if (perPageNoOfRecordsPresent > 0) {
			while (noOfRecordsChecked < totalNoOfRecords) {
				if (totalNoOfRecords > 1) {
					for (int i = 1; i <= perPageNoOfRecordsPresent; i++) {
						String newDoctReqNameInApproval = driver.findElement(
								By.xpath("//*[@id=\"terminateDocumentInitiateReqGridId\"]/div/div[4]/table/tbody/tr["
										+ i + "]/td[3]"))
								.getText();// documentTypeName
						if (documentName.equalsIgnoreCase(newDoctReqNameInApproval)) {
							driver.findElement(By
									.xpath("//*[@id=\"terminateDocumentInitiateReqGridId\"]/div/div[4]/table/tbody/tr["
											+ i + "]/td[3]"))
									.click();
				...
				if ((!isRecordSelectedChgDmtInitiation) && (noOfRecordsChecked < totalNoOfRecords)) {
					...
					table = driver.findElement(By.id("terminateDocumentInitiateReqGridId"));// Document Tree approve
																							// table
