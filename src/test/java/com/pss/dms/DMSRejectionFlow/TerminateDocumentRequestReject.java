...
public class TerminateDocumentRequestReject extends QMSLoginDetails {

	@Test
	public void TerminateDocumentRequestRejectMethod()
			throws InterruptedException, IOException, DocumentException, Exception {
//		try {

		document = new Document();
		Font font = new Font(Font.FontFamily.TIMES_ROMAN);
		output = System.getProperty("user.dir") + "\\" + "/TestReport/" + "TerminateDocumentRequestReject"
		...
		writer.open();
		HeaderFooterPageEvent event = new HeaderFooterPageEvent("TerminateDocumentRequestReject", "PSS-DMS-004",
				"Pass");
...
//        document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Click on New document request Submenu", sno,false);
		...
		toTerminateDocumentRequestRejectMethod();
		document.close();
		writer.close();
...
//	}
	...
	private void toTerminateDocumentRequestRejectMethod() throws Exception {
		Thread.sleep(1000);
		sno++;
		...
		if (isRecordSelectedForApprovalReq) {
			driver.findElement(By.id("commentsInTermDocReqApprove")).sendKeys(properties.getProperty("COMMENTSRevert"));
			document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Enter the comments", sno, false);
			Thread.sleep(1000);
			sno++;
			driver.findElement(By.id("rejectInTermDocReqApprove")).click();
