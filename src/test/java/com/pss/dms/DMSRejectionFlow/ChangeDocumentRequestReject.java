...
public class ChangeDocumentRequestReject extends QMSLoginDetails {

	@Test
	public void ChangeDocumentRequestRejectMethod()
			throws InterruptedException, IOException, DocumentException, Exception {
//		try {

		document = new Document();
		Font font = new Font(Font.FontFamily.TIMES_ROMAN);
		output = System.getProperty("user.dir") + "\\" + "/TestReport/" + "ChangeDocumentRequestReject"
		...
		writer.open();
		HeaderFooterPageEvent event = new HeaderFooterPageEvent("ChangeDocumentRequestReject", "PSS-DMS-004", "Pass");
...
//        Thread.sleep(4000);
		sno++;
		wait1.until(ExpectedConditions.elementToBeClickable(By.cssSelector("a[href='changeDocReqAppPage.do']")));
		driver.findElement(By.cssSelector("a[href='changeDocReqAppPage.do']")).click();
		Thread.sleep(2000);
		if (driver.findElement(By.id("NotificationTableContainer")).isDisplayed()) {
			System.out.println("Notification Window Displayed");
			Actions action = new Actions(driver);
...
//        document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Click on New document request Submenu", sno,false);
		...
		wait1.until(ExpectedConditions.presenceOfElementLocated(By.cssSelector(
				"#changeDocRequestApprovalTable > div > div.jtable-busy-message[style='display: none;']")));
		toChangeDocumentRequestRejectMethod();
		document.close();
		writer.close();
...
//	}
	...
	private void toChangeDocumentRequestRejectMethod() throws Exception {
		Thread.sleep(1000);
		sno++;
		...
		if (isRecordSelectedForApprovalReq) {
			driver.findElement(By.id("changeDocRequestAppComments")).sendKeys(properties.getProperty("COMMENTSRevert"));
			document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Enter the comments", sno, false);
			Thread.sleep(1000);
			sno++;
			driver.findElement(By.id("rejectBtnInchangeDocRequestApp")).click();
	...
	private boolean selectRecordDocReqApproval(String documentNameReq, boolean isRecordSelectedForApprovalReq,
		...
		WebElement table = driver.findElement(By.id("changeDocRequestApprovalTable"));
		...
		if (perPageNoOfRecordsPresent > 0 && count == 0) {
			if ((totalNoOfRecords > 1) && ((documentNameReq == null) || ("".equalsIgnoreCase(documentNameReq)))) {
				documentNameReq = driver
						.findElement(By.xpath("//*[@id=\"changeDocRequestApprovalTable\"]/div/table/tbody/tr[1]/td[3]"))
						.getText();// documentType
			} else if ((documentNameReq == null) || ("".equalsIgnoreCase(documentNameReq))) {
				documentNameReq = driver
						.findElement(By.xpath("//*[@id=\"changeDocRequestApprovalTable\"]/div/table/tbody/tr/td[3]"))
						.getText();// documentType
			}
			++count;
		}
			...
			while (noOfRecordsChecked < totalNoOfRecords) {
				if (totalNoOfRecords > 1) {
					for (int i = 1; i <= perPageNoOfRecordsPresent; i++) {
						String newDoctReqNameInApproval = driver.findElement(By.xpath(
								"//*[@id=\"changeDocRequestApprovalTable\"]/div/table/tbody/tr[" + i + "]/td[3]"))
					...
					String newDoctReqNameInApproval = driver
							.findElement(
									By.xpath("//*[@id=\"changeDocRequestApprovalTable\"]/div/table/tbody/tr/td[3]"))
							.getText();
					if (documentNameReq.equalsIgnoreCase(newDoctReqNameInApproval)) {
						driver.findElement(
								By.xpath("//*[@id=\"changeDocRequestApprovalTable\"]/div/table/tbody/tr/td[3]"))
				...
				if ((!isRecordSelectedForApprovalReq) && (noOfRecordsChecked < totalNoOfRecords)) {
					driver.findElement(By.className("jtable-page-number-next")).click();// next page in Document approve
																						// list
					Thread.sleep(3000);
					table = driver.findElement(By.id("changeDocRequestApprovalTable"));// Document Tree approve table
