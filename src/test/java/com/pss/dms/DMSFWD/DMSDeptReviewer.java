...
public class DMSDeptReviewer extends QMSLoginDetails {
	@Test
	public void toDMSDeptReviewer() throws InterruptedException, IOException, DocumentException, Exception {
//		try {

		document = new Document();
		Font font = new Font(Font.FontFamily.TIMES_ROMAN);
		output = System.getProperty("user.dir") + "\\" + "/TestReport/" + "DMSDeptReviewer" + (new Random().nextInt())
				+ ".pdf";
		...
		writer.open();
		HeaderFooterPageEvent event = new HeaderFooterPageEvent("DMSDeptReviewer", "PSS-DMS-006", "Pass");
...
//        sno++;
		...
		document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Click on New at Document Review/Approval menu", sno, false);
		sno++;
...
//    }
	...
	private void todoDMSDeptReviewer() throws Exception {
		...
		document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Select the record and click on Continue Approval", sno, false);
		sno++;
			...
			while (noOfRecordsChecked < totalNoOfRecords) {
				...
				if ((!isRecordSelectedForDeptReview) && (noOfRecordsChecked < totalNoOfRecords)) {
					...
					wait.until(ExpectedConditions.presenceOfElementLocated(By.cssSelector(
							"#dmsNewDocReviewTable > div > div.jtable-busy-message[style='display: none;']")));
