...
public class DMSQAApprover extends QMSLoginDetails {

	@Test
	public void toDMSQAApprover() throws InterruptedException, IOException, DocumentException, Exception {
//		try {

		document = new Document();
		Font font = new Font(Font.FontFamily.TIMES_ROMAN);
		output = System.getProperty("user.dir") + "\\" + "/TestReport/" + "DMSQAApprover" + (new Random().nextInt())
				+ ".pdf";
		...
		writer.open();
		HeaderFooterPageEvent event = new HeaderFooterPageEvent("DMSQAApprover", "PSS-DMS-009", "Pass");
...
//        document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Click on New Document menu", sno,false);
		...
		document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Click on New at Document Review/Approve menu", sno, false);
		sno++;
...
//    }
	...
	private void todoDMSQAApprover() throws Exception {
		
		Thread.sleep(5000);
		int count = 0;
		String documentDeptSOPReview = properties.getProperty("DOCUMENT_NAME_SOP_DOCT_REQUEST");
		document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Select the record and click on Continue Approval button", sno, false);
		sno++;
		boolean isRecordSelectedForCFTReview = false;
		isRecordSelectedForCFTReview = selectRecordQAApproverSOPStage(documentDeptSOPReview,
...
//                        		document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Click On LogOut ", sno,true);
	...
	private boolean selectRecordQAApproverSOPStage(String documentDeptSOPReview, boolean isRecordSelectedForCFTReview,
					...
					for (int i = 1; i <= perPageNoOfRecordsPresent; i++) {
						String newDoctReqNameInApproval = driver
								.findElement(By.xpath(
										"//*[@id=\"dmsNewDocReviewTable\"]/div/table/tbody/tr[ " + i + " ]/td[1]"))
