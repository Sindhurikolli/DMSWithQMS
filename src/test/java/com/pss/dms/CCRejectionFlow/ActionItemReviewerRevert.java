...
public class ActionItemReviewerRevert extends QMSLoginDetails {

	@Test
	public void toActionItemRevert() throws Exception {

//		try {

		document = new Document();
		Font font = new Font(Font.FontFamily.TIMES_ROMAN);
		output = System.getProperty("user.dir") + "\\" + "/TestReport/" + "ActionItemReject" + (new Random().nextInt())
				+ ".pdf";
		fos = new FileOutputStream(output);
		writer = PdfWriter.getInstance(document, fos);
		writer.setStrictImageSequence(true);
		writer.open();
		HeaderFooterPageEvent event = new HeaderFooterPageEvent("ActionItemReject", "PSS-QMS-046", "Pass");
		writer.setPageEvent(event);
		document.open();
		driver.get(properties.getProperty("QMSLoginUrl"));
		Thread.sleep(1000);
		driver.findElement(By.name("loginUserName")).sendKeys(properties.getProperty("CCActionItemReviewer"));
		Thread.sleep(1000);
...
//        Thread.sleep(16000);
		...
		wait1.until(ExpectedConditions.presenceOfElementLocated(By.cssSelector(
				"#changeControlActionItemReviewTable > div > div.jtable-busy-message[style='display: none;']")));
...
//        System.out.println("CC Number is coming........:"+CCNumber);
		...
		if (isRecordSelected) {
			...
			WebElement element = driver.findElement(By.id("rejBtnIdInCcActItmReview"));
			jse.executeScript("arguments[0].scrollIntoView(true);", element);
			driver.findElement(By.id("rejBtnIdInCcActItmReview")).click();
...
//                    Thread.sleep(5000);
				...
				if ((!isRecordSelected) && (noOfRecordsChecked < totalNoOfRecords)) {
					driver.findElement(By.cssSelector(
							"#changeControlActionItemReviewTable > div > div.jtable-bottom-panel > div.jtable-left-area > span.jtable-page-list > span.jtable-page-number-next"))
							.click();// next page in Document approve list
					Thread.sleep(3000);
					table = driver.findElement(By.id("changeControlActionItemReviewTable"));// Document Tree approve
																							// table
