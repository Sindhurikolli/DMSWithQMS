...
public class QAApproverReject extends QMSLoginDetails {
	...
	@Test
	public void toQAApproveReject() throws Exception {

//		try {

		document = new Document();
		Font font = new Font(Font.FontFamily.TIMES_ROMAN);
		output = System.getProperty("user.dir") + "\\" + "/TestReport/" + "QAApproverReject" + (new Random().nextInt())
				+ ".pdf";
		fos = new FileOutputStream(output);
		writer = PdfWriter.getInstance(document, fos);
		writer.setStrictImageSequence(true);
		writer.open();
		HeaderFooterPageEvent event = new HeaderFooterPageEvent("QAApproverReject", "PSS-QMS-035", "Pass");
		writer.setPageEvent(event);
		document.open();
		driver.get(properties.getProperty("QMSLoginUrl"));
		Thread.sleep(1000);
		driver.findElement(By.name("loginUserName")).sendKeys(properties.getProperty("CCQAhod"));
		Thread.sleep(1000);
...
//	            Thread.sleep(1000);
			...
			document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Click on Revert Radio button", sno, false);
			Thread.sleep(3000);
			sno++;
