...
public class ActionItemApproveReject extends QMSLoginDetails {

	@Test
	public void toActionItemApproveReject() throws Exception {

//		try {

		document = new Document();
		Font font = new Font(Font.FontFamily.TIMES_ROMAN);
		output = System.getProperty("user.dir") + "\\" + "/TestReport/" + "ActionItemApproveReject"
				+ (new Random().nextInt()) + ".pdf";
		fos = new FileOutputStream(output);
		writer = PdfWriter.getInstance(document, fos);
		writer.setStrictImageSequence(true);
		writer.open();
		HeaderFooterPageEvent event = new HeaderFooterPageEvent("ActionItemApproveReject", "PSS-QMS-049", "Pass");
...
//	        System.out.println("ChangeControl Number is coming........:"+chgControlNumber);
		...
		if (isRecordSelected) {
			Thread.sleep(3000);
			sno++;
			driver.findElement(By.id("commentsInApproveForm"))
					.sendKeys(properties.getProperty("CHANGE_CONTROL_SHORT_DESC"));
			document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Enter the comments", sno, false);
			Thread.sleep(3000);
			sno++;
			JavascriptExecutor jse = (JavascriptExecutor) driver;
			WebElement element = driver.findElement(By.id("rejBtnIdInCcActItmApp"));
			jse.executeScript("arguments[0].scrollIntoView(true);", element);
			driver.findElement(By.id("rejBtnIdInCcActItmApp")).click();
	...
	private boolean selectRecdActionItemApprove(String CCNumber, boolean isRecordSelected, int count)
