...
public class AdhocSchedule extends QMSLoginDetails {
...
//		jse1.executeScript("arguments[0].scrollIntoView(true);", element1);
	...
	private void methodToCreateAdhocSchedule() throws Exception {
		...
		document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Click on Location", sno, false);
		Thread.sleep(6000);
		wait1.until(ExpectedConditions.presenceOfElementLocated(By.cssSelector("#workFlowJtable > div > div.jtable-busy-message[style='display: none;']")));
		int count = 0;
		...
		if (isRecordSelected) {
			...
			wait.until(ExpectedConditions.presenceOfElementLocated(By.cssSelector("#MultiItemTableContainer > div > div.jtable-busy-message[style='display: none;']")));
			Thread.sleep(4000);
			...
			document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Click on Search", sno, false);
			Thread.sleep(5000);
			wait1.until(ExpectedConditions.presenceOfElementLocated(By.cssSelector("#MultiItemTableContainer > div > div.jtable-busy-message[style='display: none;']")));
			int count2 = 0;
