...
public class CreateTopic extends QMSLoginDetails {
	...
	public void toCreateTopic() throws Exception {
		...
		output = System.getProperty("user.dir") + "\\" + "/TestReport/" + "CreateItem" + (new Random().nextInt())
				+ ".pdf";
		...
		writer.open();
		HeaderFooterPageEvent event = new HeaderFooterPageEvent("Create Item", "PSS-LMS-003", "Pass");
...
//		driver.findElement(By.id("createItem")).click();
		document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Click on Create Item", sno, false);
		Thread.sleep(4000);
		methodToCreateTopic();
		document.close();
		writer.close();
	...
	private String getVersionNumber(int count, boolean isRecordSelected, String name, String versionNumber)
		...
		if (perPageNoOfRecordsPresent > 0) {
			while (noOfRecordsChecked < totalNoOfRecords) {
				...
				noOfRecordsChecked += perPageNoOfRecordsPresent;
				if ((!isRecordSelected) && (noOfRecordsChecked < totalNoOfRecords)) {
					driver.findElement(By.cssSelector("#newDocDetailsTable > div > div.jtable-bottom-panel > div.jtable-left-area > span.jtable-page-list > span.jtable-page-number-next")).click();// next page in Document approve
																						// list
					Thread.sleep(8000);
					table = driver.findElement(By.id("newDocDetailsTable"));// Document approve table
					tableBody = table.findElement(By.tagName("tbody"));
