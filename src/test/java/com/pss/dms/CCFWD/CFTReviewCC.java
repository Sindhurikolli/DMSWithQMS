...
public class CFTReviewCC extends QMSLoginDetails {

	@Test
	public void toCFTReviewCC() throws InterruptedException, IOException, DocumentException, Exception {
//		try {

		document = new Document();
		Font font = new Font(Font.FontFamily.TIMES_ROMAN);
		output = System.getProperty("user.dir") + "\\" + "/TestReport/" + "CFTReviewCC" + (new Random().nextInt())
				+ ".pdf";
		...
		writer.open();
		HeaderFooterPageEvent event = new HeaderFooterPageEvent("CFTReviewCC", "PSS-QMS-004", "Pass");
...
//        document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Click on My Activities Tab", sno,false);
		...
		wait1.until(ExpectedConditions.presenceOfElementLocated(By.cssSelector("a[href='ccCftReviewPage.do']")));
...
//	        System.out.println("CC Number is coming........:"+CCNumber);
		...
		if (isRecordSelected) {
			...
			Thread.sleep(5000);
			sno++;
			driver.findElement(By.id("suppDocInCcCftReviewWindow")).sendKeys(properties.getProperty("Document-1"));
			...
			document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Select Accept Radio button and click on Submit", sno, false);
			Thread.sleep(5000);
			sno++;
	...
	private boolean selectRecdCftReviewChgControl(String chgControlNumber, boolean isRecordSelected, int count)
			...
			while (noOfRecordsChecked < totalNoOfRecords) {
				if (totalNoOfRecords > 1) {
					for (int i = 1; i <= perPageNoOfRecordsPresent; i++) {
						...
						WebElement ccNumber = driver.findElement(
								By.xpath(".//*[@id='changeControlCftReviewTableContainer']/div/table/tbody/tr[ " + i
										+ "]/td[3]"));
						js.executeScript("arguments[0].scrollIntoView();", ccNumber);
						Thread.sleep(1000);
						...
						if (chgControlNumber.equalsIgnoreCase(CCNumberSequence)) {
							// driver.findElement(By.xpath(".//*[@id='changeControlCftReviewTableContainer']/div/table/tbody/tr["
							// + i + "]/td[53]/button")).click();
				...
				if ((!isRecordSelected) && (noOfRecordsChecked < totalNoOfRecords)) {
					JavascriptExecutor js = (JavascriptExecutor) driver;
					WebElement ccNumber = driver.findElement(By.cssSelector(
