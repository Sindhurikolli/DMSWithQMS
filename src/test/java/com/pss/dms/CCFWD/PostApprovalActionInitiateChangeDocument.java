...
public class PostApprovalActionInitiateChangeDocument extends QMSLoginDetails {
	@Test
...
//
			...
			document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Click on Add Documents Check Box", sno, false);
			sno++;
			Thread.sleep(5000);
			...
			Thread.sleep(3000);
			driver.findElement(By.id("searchDocName")).sendKeys(properties.getProperty("DOCUMENT_NAME_SOP_DOCT_REQUEST"));
			document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Enter Document Name", sno, false);
			sno++;
			Thread.sleep(1000);
			driver.findElement(By.id("searchBtnInDocumentAdd")).click();
			Thread.sleep(3000);
			document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Click on Search", sno, false);
			sno++;
			wait.until(ExpectedConditions.presenceOfElementLocated(By.cssSelector("#documentsWidowDetialsTableContainer > div > div.jtable-busy-message[style='display: none;']")));
			
			int count1 = 0;
			boolean isRecordSelectedForChngDoc = false;
//			String doctype = (String) properties.getProperty("DocumentType");
//			Map changeDocDetails = selectRecdForDocType(doctype, isRecordSelectedForChngDoc, count1);
//			System.out.println(changeDocDetails.get("name"));
//			System.out.println(changeDocDetails.get("no"));
//			PropertiesConfiguration properties = new PropertiesConfiguration(
//					"src/test/java/com/pss/dms/properties/DMSProperties.properties");
//			properties.setProperty("DOCUMENT_NAME_SOP_DOCT_REQUEST", changeDocDetails.get("name"));
//			properties.setProperty("DOCT_NO_INITIATE_NEW_DOCT_SOP", changeDocDetails.get("no"));
//			properties.save();
		...
		if (perPageNoOfRecordsPresent > 0) {
			while (noOfRecordsChecked < totalNoOfRecords) {
				...
				if ((!isRecordSelectedForChngDoc) && (noOfRecordsChecked < totalNoOfRecords)) {
					driver.findElement(By.cssSelector("#documentsWidowDetialsTableContainer > div > div.jtable-bottom-panel > div.jtable-left-area > span.jtable-page-list > span.jtable-page-number-next")).click();// next page in Document approve list
					Thread.sleep(3000);
					table = driver.findElement(By.id("documentsWidowDetialsTableContainer"));// Document Tree approve																								// table
					tableBody = table.findElement(By.tagName("tbody"));
					perPageNoOfRecordsPresent = tableBody.findElements(By.tagName("tr")).size();
				}
			}
