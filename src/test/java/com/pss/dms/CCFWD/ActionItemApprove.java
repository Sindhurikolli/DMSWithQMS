...
public class ActionItemApprove extends QMSLoginDetails {

	@Test
	public void toActionItemApprove() throws Exception {

//		try {

		document = new Document();
		Font font = new Font(Font.FontFamily.TIMES_ROMAN);
		output = System.getProperty("user.dir") + "\\" + "/TestReport/" + "ActionItemApprove" + (new Random().nextInt())
				+ ".pdf";
		fos = new FileOutputStream(output);
		writer = PdfWriter.getInstance(document, fos);
		writer.setStrictImageSequence(true);
		writer.open();
		HeaderFooterPageEvent event = new HeaderFooterPageEvent("ActionItemApprove", "PSS-QMS-015", "Pass");
...
//        Thread.sleep(16000);
		...
		wait1.until(ExpectedConditions.elementToBeClickable(By.cssSelector("a[href='ccActItemAppPage.do']")));
		document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Click on Action Item Approval menu", sno, false);
		driver.findElement(By.cssSelector("a[href='ccActItemAppPage.do']")).click();
		
//        Thread.sleep(100000);
		wait1.until(ExpectedConditions.presenceOfElementLocated(By.cssSelector(
				"#changeControlActionItemApprovalTable > div > div.jtable-busy-message[style='display: none;']")));
		methodToDoActionItemApprove();
		document.close();
		writer.close();
...
//	}
	...
	private void methodToDoActionItemApprove() throws Exception {
...
//        System.out.println("CC Number is coming........:"+CCNumber);
		Thread.sleep(3000);
		isRecordSelected = selectRecdActionItemApprove(CCNumber, isRecordSelected, count);
		document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Select the record", sno, false);
		Thread.sleep(3000);
		if (isRecordSelected) {
			Thread.sleep(3000);
			sno++;
			driver.findElement(By.id("commentsInApproveForm")).sendKeys(properties.getProperty("CC_1500"));
			document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Enter the comments", sno, false);
			Thread.sleep(3000);
			sno++;
			JavascriptExecutor jse = (JavascriptExecutor) driver;
			WebElement element = driver.findElement(By.id("appBtnIdInCcActItmApp"));
			jse.executeScript("arguments[0].scrollIntoView(true);", element);
			Thread.sleep(2000);
			driver.findElement(By.id("appBtnIdInCcActItmApp")).click();
	...
	private boolean selectRecdActionItemApprove(String chgCtrlId, boolean isRecordSelected, int count)
			throws InterruptedException {
		WebElement table = driver.findElement(By.id("changeControlActionItemApprovalTable"));
		...
		if (perPageNoOfRecordsPresent > 0) {
			String a = driver
					.findElement(By.xpath("//*[@id=\"changeControlActionItemApprovalTable\"]/div/div[4]/div[2]/span"))
		...
		if (perPageNoOfRecordsPresent > 0 && count == 0) {
			if ((totalNoOfRecords > 1) && ((chgCtrlId == null) || ("".equalsIgnoreCase(chgCtrlId)))) {
				chgCtrlId = driver
						.findElement(
								By.xpath("//*[@id=\"changeControlActionItemApprovalTable\"]/div/table/tbody/tr/td[3]"))
				...
				if (totalNoOfRecords > 1) {
					for (int i = 1; i <= perPageNoOfRecordsPresent; i++) {
						String CCNumberSequence = driver.findElement(
								By.xpath("//*[@id=\"changeControlActionItemApprovalTable\"]/div/table/tbody/tr[ " + i
										+ " ]/td[3]"))
...
//                        Thread.sleep(10000);
						if (chgCtrlId.equalsIgnoreCase(CCNumberSequence)) {
							driver.findElement(
									By.xpath("//*[@id=\"changeControlActionItemApprovalTable\"]/div/table/tbody/tr[ "
											+ i + " ]/td[3]"))
									.click();
					...
					String CCNumberSequence = driver
							.findElement(By.xpath(
									"//*[@id=\"changeControlActionItemApprovalTable\"]/div/table/tbody/tr/td[3]"))
							.getText();
//                    String CCString="";
//                    String separateCCNumber[] = capaNumberSequence.split("/");
//                    System.out.println("separateCCNumber: "+separateCCNumber[0]);
