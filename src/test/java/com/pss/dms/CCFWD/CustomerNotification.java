...
public class CustomerNotification extends QMSLoginDetails {
	...
	@Test
	public void toCustomerNotification() throws InterruptedException, IOException, DocumentException, Exception {
//		try {
		...
		writer.open();
		HeaderFooterPageEvent event = new HeaderFooterPageEvent("QAReviewCustomerNotification", "PSS-QMS-006", "Pass");
		...
		document.add(new Paragraph("                                     "));
		sno++;
//			wait1.until(ExpectedConditions.presenceOfAllElementsLocatedBy(By.cssSelector("#changeControl_tile_Id > div > div > div > h2")));
...
//        System.out.println("CC Number is coming........:"+CCNumber);
		...
		if (isRecordSelected) {
			...
			document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Click On Notify customer button",sno, false);
			sno++;
			...
			WebElement elemen = driver.findElement(By.id("notifyBtnInCcNotifyCustWindow"));
