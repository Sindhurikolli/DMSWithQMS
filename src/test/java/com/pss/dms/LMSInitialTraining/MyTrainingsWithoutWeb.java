...
public class MyTrainingsWithoutWeb extends QMSLoginDetails {

	@Test
	public void myTrainings() throws Exception {

		document = new Document();
		Font font = new Font(Font.FontFamily.TIMES_ROMAN);
		output = System.getProperty("user.dir") + "\\" + "/TestReport/" + "MyTrainingsWithoutWebBased"
		...
		writer.open();
		HeaderFooterPageEvent event = new HeaderFooterPageEvent("MY TRAININGS", "PSS-LMS-009", "Pass");
		writer.setPageEvent(event);
		document.open();
		Thread.sleep(1000);
		driver.get(properties.getProperty("LMSURL"));
		Thread.sleep(2000);
		driver.findElement(By.name("loginUserName")).sendKeys(properties.getProperty("LMSNewUser"));
		Thread.sleep(1000);
...
//		WebElement element1 = driver.findElement(By.id("trainingsList"));
//		jse1.executeScript("arguments[0].scrollIntoView(true);", element1);
	...
	private void methodForMyTrainings() throws Exception {

		int count = 0;
		boolean isRecordSelected = false;
		String name = properties.getProperty("ModifyAdhocSchedule_Name");
		...
		if (isRecordSelected) {
			...
			while (itr.hasNext()) {
				...
				if (!mainWindow.equals(childWindow)) {
					...
					sno++;
					driver.findElement(By.id("buttonInMyTrainings")).click();
					document = Utilities.getScreenShotAndAddInLogDoc(driver, document,
							"click on Reading completed button", sno, false);
					Thread.sleep(3000);
				}
			}
			...
			String name1 = properties.getProperty("ModifyAdhocSchedule_Name");
			isRecordSelected1 = selectRecordToComplete(count1, isRecordSelected1, name1);
			sno++;
			document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Select a Record", sno, false);
			Thread.sleep(4000);
			sno++;
			...
			Thread.sleep(2000);
			sno++;
			driver.findElement(By.id("commentsInViewTrainingForm"))
					.sendKeys(properties.getProperty("Approval_Comments"));
			document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Enter Comments", sno, false);
			Thread.sleep(2000);
			sno++;
			driver.findElement(By.id("submitButtonInMyTrainings")).click();
	...
	private boolean selectRecordToComplete(int count1, boolean isRecordSelected1, String name1) throws Exception {
		...
		if (perPageNoOfRecordsPresent > 0 && count1 == 0) {
			if ((totalNoOfRecords > 1) && ((name1 == null) || ("".equalsIgnoreCase(name1)))) {
				name1 = driver.findElement(By.xpath("//*[@id=\"LMSMyTrainingsJtable\"]/div/table/tbody/tr[1]/td[4]"))
						.getText();// Doc No
			} else if ((name1 == null) || ("".equalsIgnoreCase(name1))) {
				name1 = driver.findElement(By.xpath("//*[@id=\"LMSMyTrainingsJtable\"]/div/table/tbody/tr/td[4]"))
						.getText();// Doc
				// No
			}
			++count1;
