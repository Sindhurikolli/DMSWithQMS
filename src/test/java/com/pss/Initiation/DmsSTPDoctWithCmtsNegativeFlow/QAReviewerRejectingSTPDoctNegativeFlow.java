 ...
 * @author Jeevan
 */
public class QAReviewerRejectingSTPDoctNegativeFlow extends QMSLoginDetails {

	public QAReviewerRejectingSTPDoctNegativeFlow() {
	}
	...
	@Test
	public void toDoQAReviewerRejectingSTPDoctNegativeFlow() throws InterruptedException {
		Thread.sleep(1000);
		...
		Thread.sleep(1000);
		metodToDoQAReviewerSTPRejectingDocs();

	}

	private void metodToDoQAReviewerSTPRejectingDocs() throws InterruptedException {
		int count = 0;
		String documentQASTPReview = properties.getProperty("DOCUMENT_NAME_STP_DOCT_REQUEST_REJECT_FLOW");
		boolean isRecordSelectedForQAReview = false;
		isRecordSelectedForQAReview = selectRecordQAReviewStageSTPDoctNegativeFlow(documentQASTPReview,
		...
		if (isRecordSelectedForQAReview) {
			...
			for (String pdfWindows : pdfWindow) {
				...
				if (!applicationWindow.equalsIgnoreCase(pdfWindows)) {
					...
					Thread.sleep(1000);
					driver.findElement(
							By.cssSelector("#annotationsToolbar > div.flowpaper_bttnComment.flowpaper_tblabelbutton"))
							.click();
					Thread.sleep(1000);
					driver.findElement(
	...
	private boolean selectRecordQAReviewStageSTPDoctNegativeFlow(String documentQASTPReview,
		...
		if (perPageNoOfRecordsPresent > 0 && count == 0) {
			if ((totalNoOfRecords > 1)
					&& ((documentQASTPReview == null) || ("".equalsIgnoreCase(documentQASTPReview)))) {
				documentQASTPReview = driver
						.findElement(By.xpath("//*[@id=\"dmsNewDocReviewTable\"]/div/table/tbody/tr[1]/td[1]"))
						.getText();// documentType
			} else if ((documentQASTPReview == null) || ("".equalsIgnoreCase(documentQASTPReview))) {
				documentQASTPReview = driver
			...
			while (noOfRecordsChecked < totalNoOfRecords) {
				if (totalNoOfRecords > 1) {
					for (int i = 1; i <= perPageNoOfRecordsPresent; i++) {
						...
						if (documentQASTPReview.equalsIgnoreCase(newDoctReqNameInApproval)) {
							driver.findElement(
