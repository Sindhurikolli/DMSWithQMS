 ...
 * @author Jeevan
 */
public class DocumentViewSTPDoctRejectNegativeFlow extends QMSLoginDetails {

	public DocumentViewSTPDoctRejectNegativeFlow() {
	}
	...
	//
	@Test
	public void toDoDocumentViewSTPDoctRejectNegativeFlow() throws InterruptedException {
		Thread.sleep(1000);
		...
		Thread.sleep(1000);
		methodToDoSearchViewSTPDoctRejectNegativeFlow();
	}

	private void methodToDoSearchViewSTPDoctRejectNegativeFlow() throws InterruptedException {
		driver.findElement(By.id("dmsDocViewListSearchBtnId")).click();
		Thread.sleep(1000);
		String doctNameView = properties.getProperty("DOCUMENT_NAME_STP_DOCT_REQUEST_REJECT_FLOW");
		boolean isRecordSelected = false;
		int count = 0;
		String applicationWindow = driver.getWindowHandle();
		Thread.sleep(1000);
		isRecordSelected = selectDoctRecdViewSTPDoctNegativeFlow(doctNameView, isRecordSelected, count);
		Thread.sleep(2000);
		if (isRecordSelected) {
	...
	private boolean selectDoctRecdViewSTPDoctNegativeFlow(String doctNameView, boolean isRecordSelected, int count)
