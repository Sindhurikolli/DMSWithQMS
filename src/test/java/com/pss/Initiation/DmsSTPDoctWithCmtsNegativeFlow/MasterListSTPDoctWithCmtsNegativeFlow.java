 ...
 * @author Jeevan
 */
public class MasterListSTPDoctWithCmtsNegativeFlow extends QMSLoginDetails {

	public MasterListSTPDoctWithCmtsNegativeFlow() {
	}
	...
	@Test
	public void toDoMasterListSTPDoctWithCmtsNegativeFlow() throws InterruptedException {
		Thread.sleep(1000);
		driver.findElement(By.id("bmrHomeInCommonHPHeader")).click();
		Thread.sleep(1000);
		driver.findElement(By.id("masterListTabIdInDMS")).click();
		Thread.sleep(1000);
		methodToViewMasterListDoctSTPRejectFlow();
	}

	private void methodToViewMasterListDoctSTPRejectFlow() throws InterruptedException {
		driver.findElement(By.id("tree_1_span")).click();
		Thread.sleep(1000);
		boolean isRecordSelected = false;
		int count = 0;
		String doctName = properties.getProperty("DOCUMENT_NAME_STP_DOCT_REQUEST_REJECT_FLOW");
		String applicationWindow = driver.getWindowHandle();
		Thread.sleep(1000);
		isRecordSelected = selectRecordToViewMasterListDoctSTPRejectFlow(doctName, isRecordSelected, count);
		Thread.sleep(1000);
		if (isRecordSelected) {
	...
	private boolean selectRecordToViewMasterListDoctSTPRejectFlow(String doctName, boolean isRecordSelected, int count)
